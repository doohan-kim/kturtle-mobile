...
from market_scanner import get_kr_universe, scan_market, select_top_sector_leaders
...
with tab1:
    ...
    if st.button("한국시장 전체 스캔", type="primary"):
        try:
            ...
            st.session_state["kt_sector_strength"] = sector_strength
    ...
    if "kt_sector_strength" in st.session_state:
        ...
        if not sec.empty:
            st.subheader("🔥 시장 강세 섹터 TOP 3")
            for _, r in sec.iterrows():
                st.markdown(
                    f'**{int(r["SectorRank"])}위 {r["Sector"]}** · '
                    f'섹터점수 **{r["SectorScore"]:.1f}** · '
                    f'20일 {r["SectorRet20"]:.1f}% / 60일 {r["SectorRet60"]:.1f}%'
                )
    ...
    if "kt_candidates" in st.session_state:
        ...
        if cand.empty:
            ...
            st.caption("전체 섹터를 먼저 비교해 상대강도 상위 3개 섹터만 남기고, 각 섹터의 돌파 후보 중 RS가 가장 강한 1종목을 대장주로 표시합니다.")
            ...
            if st.button("선택 후보 최종 검증", type="primary"):
                ...
                try:
                    with st.spinner("돌파 대장주의 DART 재무·공시를 확인하는 중..."):
                        gate, reasons, counts, corp, name = dart_gate(selected_code)
                except Exception as e:
...
st.caption(
    "v2.0: 전체 섹터 20일·60일 수익률 중앙값으로 섹터 강도를 비교해 TOP 3만 선택하고, "
