...
from financial_db import get_stock_finance, db_meta
...
st.title("🐢 K‑TURTLE Mobile v2.4")
st.caption("가격 원자료 검증 → 매매계획 → Heat → 재무 Gate → 삼성증권 주문 준비")
meta = db_meta()
st.caption(f"재무 DB: {meta.get('stock_count',0):,}종목 · 갱신 {meta.get('generated_at') or '아직 없음'}")
    ...
    if "kt_candidates" in st.session_state:
        ...
        if cand.empty:
            ...
            if st.button("선택 후보 최종 검증", type="primary"):
                ...
                # 1) Daily local DB first
                db_item, db_age = get_stock_finance(selected_code)
                source = "DB"
                live_error = None

                if db_item and db_age is not None and db_age <= 24 * 7:
                    gate = db_item.get("gate", "WATCH")
                    reasons = db_item.get("reasons", [])
                    counts = db_item.get("financing", {})
                    corp = db_item.get("corp_code")
                    name = db_item.get("name", selected_row.get("Name", selected_code))

                    st.success(
                        f"📦 저장 재무 DB 사용 · {db_age:.1f}시간 전 갱신"
                    )
                else:
                    # 2) DB missing/stale -> live DART fallback
                    try:
                        with st.spinner("저장 DB가 없거나 오래되어 OpenDART를 확인하는 중..."):
                            gate, reasons, counts, corp, name, _, _ = dart_gate(selected_code)
                        source = "LIVE"
                        st.success("🌐 OpenDART 실시간 재무 확인")
                    except Exception as e:
                        live_error = str(e)

                        # 3) last temp cache fallback
                        cached = load_financial_gate(selected_code)
                        if cached:
                            ...
                            if age is None or age > 24 * 7:
                                gate = "WATCH"
                                reasons = list(reasons) + ["재무 캐시 7일 초과 — 최신 확인 필요"]
                                st.warning("🟡 오래된 임시 캐시 — 신규 진입 보류")
                            else:
                                st.warning(f"⚠️ 임시 캐시 사용 · {age:.1f}시간 전")
                        else:
                            gate = "WATCH"
                            reasons = ["재무 DB 없음 + OpenDART 연결 실패"]
                            counts = {}
                            corp = None
                ...
                if live_error:
                    st.caption(f"OpenDART 오류: {live_error}")
...
st.caption(
    "v2.4: 저장 재무 DB 우선 → DB 없거나 7일 초과 시 OpenDART 실시간 → "
    "실패 시 임시 캐시 → 그래도 없으면 WATCH."
)
