...
with tab1:
    ...
    if "kt_candidates" in st.session_state:
        ...
        if cand.empty:
            ...
            st.caption("모바일에서는 후보를 카드로 표시합니다. 원자료 표는 아래에서 펼쳐볼 수 있습니다.")
            ...
            for idx, row in cand.head(int(max_candidates)).iterrows():
                ...
                stop = row.get("Stop2N")
                add = row.get("Add0_5N")
                b20 = row.get("Break20Pct")
                b55 = row.get("Break55Pct")

                st.markdown(
                    f"""
                    <div class="kt-card">
                      <div class="kt-title">{name} <span style="opacity:.6;font-size:.9rem">({code})</span></div>
                      <div class="kt-sub">{market} · {breakout}</div>
                      <div class="kt-grid">
            ...
            with st.expander("📊 원자료 표 보기"):
                ...
                rename = {
                    "Code":"코드","Name":"종목","Market":"시장","Breakout":"돌파",
