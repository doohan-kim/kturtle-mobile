    ...
    if "kt_candidates" in st.session_state:
        ...
        if cand.empty:
            ...
            if st.button("선택 후보 최종 검증", type="primary"):
                ...
                try:
                    with st.spinner("OpenDART 재무·공시를 확인하는 중..."):
                        gate, reasons, counts, corp, name, source, cache_hours = dart_gate(selected_code)
                except Exception as e:
                    live_error = str(e)
                    ...
                    if cached:
                        ...
                        cache_hours = age if age is not None else 999999
                        ...
                        source = "CACHE"
                        st.warning(f"⚠️ OpenDART 실시간 연결 실패 — 최근 캐시 사용 ({cache_hours:.1f}시간 전)")
                        st.caption(f"실시간 오류: {live_error}")
                    else:
                        gate = "WATCH"
                        reasons = ["OpenDART 연결 실패 + 저장된 재무 캐시 없음"]
                        counts = {}
                        corp = None
                ...
                max_cache_hours = 24 * 7
                if source == "CACHE" and cache_hours > max_cache_hours:
                    gate = "WATCH"
                    reasons = list(reasons) + ["재무 캐시 7일 초과 — 최신 DART 재확인 필요"]
                    st.warning(f"🟡 재무 캐시가 {cache_hours/24:.1f}일 경과했습니다. 재검증 전 신규 진입 보류")

                if gate == "PASS":
                    st.success("🟢 재무 Gate PASS · DART 실시간" if source == "LIVE" else "🟢 재무 Gate PASS · 최근 캐시")
                elif gate == "WATCH":
                ...
                if source == "CACHE":
                    st.info("캐시 데이터는 DART 장애 시 임시 확인용입니다. 7일 이내 캐시는 기존 판정을 유지하지만 가능하면 실시간 재검증 후 주문하세요.")
...
st.caption("v2.3: OpenDART 실시간 조회 → 실패 시 종목별 최근 캐시 → 캐시 없음 또는 7일 초과 시 WATCH. 미확정 상태를 PASS로 처리하지 않습니다.")
