...
def breakout_sector_top3(candidates: pd.DataFrame, sector_strength: pd.DataFrame, top_n: int = 3) -> pd.DataFrame:
    """가격 Gate 통과 종목이 존재하는 섹터만 대상으로 강도 TOP N 반환."""
    if candidates is None or candidates.empty or sector_strength is None or sector_strength.empty:
        return pd.DataFrame()
    active = candidates["Sector"].dropna().astype(str).unique()
    out = sector_strength[sector_strength["Sector"].astype(str).isin(active)].copy()
    out = out.sort_values(
        ["SectorScore", "SectorRet60", "SectorRet20"],
        ascending=[False, False, False]
